...
Cells were divided into groups based on ovalbumin signal. The fraction of cells belonging to each group is shown for each sample. 
```{r "{{.x}} OVA cluster summary", fig.width = 6.4, fig.height = 4}
...
ova_clusts <- sobj %>%
  FetchData(c("ova_group", "ova_sam_grp")) %>%
  unique() %>%
  arrange(ova_group) %>%
  pull(ova_sam_grp)
