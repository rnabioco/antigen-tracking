...
```{r "{{.x}} Sample OVA", fig.width = 8.5, fig.height = 4}
...
ova_boxes <- sobj %>%
  ...
  geom_quasirandom(size = 1.2, color = "black") +
  geom_quasirandom(size = 0.5) +
  stat_summary(fun = "median", geom = "point", shape = 21, size = 3, show.legend = F, fill = "white", color = "black", stroke = 0.5) +
...
```{r "{{.x}} Cell type OVA", fig.width = 8.5, fig.height = 7.3}
...
```{r "{{.x}} Cell type cluster markers", fig.width = 8.5, fig.height = 14, results = "asis"}
...
# Cluster plots
for (i in seq_along(sam_type_cols)) {
  cat("\n#### ", names(sam_type_cols)[i], "\n", sep = "")
  
  sep_marks <- cell_type_marks %>%
    filter(cluster == names(sam_type_cols)[i])

  type <- names(sam_type_cols)[i] %>%
  ...
  sep_cols <- c( "Other" = "#fafafa", sep_cols)
  ...
  sep_clust_umap <- sobj %>%
    ...
    guides(color = guide_legend(override.aes = list(size = 3.5, shape = 21, color = "black", stroke = 0.25))) +
    scale_fill_manual(values = sep_cols) +
    blank_theme +
    theme(
      legend.position = "top",
      legend.title    = element_blank(),
...
```{r "{{.x}} OVA cluster summary", fig.width = 6.4, fig.height = 4.3}
...
# ova_clusts <- ova_sam_markers %>%
#   mutate(ova_num = str_extract(cluster, "[0-9]+$")) %>%
#   arrange(ova_num) %>%
#   pull(cluster) %>%
#   unique()

# OVA group plots
for (i in seq_along(ova_sam_cols)) {
  cat("\n#### ", names(ova_sam_cols)[i], "\n", sep = "")
  
  sep_marks <- ova_sam_markers %>%
    filter(cluster == names(ova_sam_cols)[i])

  type <- names(ova_sam_cols)[i] %>%
    str_remove("^[a-zA-Z0-9_]+-")
