...
ova_boxes <- sobj %>%
  ...
  mutate(cell_type = fct_reorder(cell_type, adt_ovalbumin, median)) %>%
  ggplot(aes(cell_type, adt_ovalbumin + 0.01, color = cell_type)) +
...
ova_boxes <- sobj %>%
  FetchData(c("adt_ovalbumin", "cell_type", "cluster_id")) %>%
  as_tibble(rownames = "cell_id") %>%
  mutate(cluster_id = fct_reorder(cluster_id, adt_ovalbumin, median)) %>%
  ggplot(aes(cluster_id, adt_ovalbumin + 0.01, color = cluster_id)) +
...
### Cell Type Ovalbumin Markers

```{r "{{.y}} Cell type OVA groups summary", fig.width = 8.5, fig.height = 6}

# Divide cell types based on OVA signal
sobj <- sobj %>%
...
ova_boxes <- ova_data %>%
  ggplot(aes(cell_type, adt_ovalbumin + 0.01, color = type_ova_grp)) +
  geom_quasirandom(size = 0.25 * mtplyr_2) +
...
```{r "{{.y}} Cell type OVA markers", fig.width = 8.5, fig.height = 14, results = "asis"}

# Find OVA markers separately for each cell type
fig_markers <- cell_types %>%
  map(~ {
    find_group_markers(
      input_grp  = .x,
