...
ova_hist <- gmm_res$res %>%
  ...
  stat_function(
    geom  = "line",
    fun   = plot_mix_comps,
    args  = list(gmm_res$mu[1], gmm_res$sigma[1], gmm_res$lambda[1]),
    color = ova_cols[1],
    lwd   = 1
  ) +
  stat_function(
    geom  = "line",
    fun   = plot_mix_comps,
    args  = list(gmm_res$mu[2], gmm_res$sigma[2], gmm_res$lambda[2]),
    color = ova_cols[2],
    lwd   = 1
  ) +
...
# Set OVA colors
type_ova_cols <- theme_cols_threes %>%
  reduce(c)
...
type_ova_cols <- type_ova_cols[!is.na(names(type_ova_cols))]
