...
create_marker_umaps <- function(input_sobj, input_markers, umap_col = NULL, add_outline = NULL, 
                                pt_mtplyr = 1, low_col = "#fafafa", high_col_mtplyr = 1) {
...
walk2(sobjs, NEW_sobjs, ~ {
  so1 <- .x
  so2 <- .y
  ident_columns <- colnames(so1@meta.data)
  walk(ident_columns, ~ {
    iden <- identical(so1@meta.data[, .x], so2@meta.data[, .x])
