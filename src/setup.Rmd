...
create_sobj <- function(matrix_dir, proj_name = "SeuratProject", hash_ids = NULL, adt_count_min = 50,
...
# OLD Subset Seurat objects for plotting
subset_sobj_OLD <- function(sobj_in, name, type_vec, include_list, type_column = "cell_type1", subtype_column = "cell_type2",
...
subset_sobj <- function(sobj_in, name, cell_types, type_column = "cell_type", ...) {
  ...
  # Filter cells based on input cell type
  res <- res %>%
    subset(subset = cell_type %in% cell_types)
...
run_clustifyr <- function(sobj_in, type_in, ref, resolution = 1.8, type_column = "cell_type",
...
ref_sobjs <- ref_sobjs %>%
  map(~ here(params$rds_dir, .x)) %>%  # Use map to keep names
  map(read_rds)

ref_so <- merge(ref_sobjs[[1]], ref_sobjs[2:length(ref_sobjs)])

# Cell type refs
type_ref <- ref_so %>%
  seurat_ref(cluster_col = "cell_type1")

# Cell subtype refs
# Create separate subtype ref for each cell type
cell_types <- unique(ref_so$cell_type1)

subtype_refs <- cell_types %>%
  set_names(., .) %>%
  map(~ {
    ref_so %>%
      subset(subset = cell_type1 == .x) %>%
      seurat_ref(cluster_col = "cell_type2")
  })
...
#   FRC = "fibroblast"
# )
# Separate refs for each timepoint
# arg_tbl <- expand_grid(objs, types)
# 
# names(arg_tbl$objs) <- str_c(names(arg_tbl$objs), "_", names(arg_tbl$types))
# 
# refs <- map2(arg_tbl$objs, arg_tbl$types, ~ {
#   
#   res <- here(params$rds_dir, .x) %>%
#     read_rds() %>%
#     subset(subset = cell_type1 == .y) %>%
#     seurat_ref(cluster_col = "cell_type2")
#   
#   res
# })
# Combined ref for timepoints
# refs <- imap(types, ~ {
#   so_type <- .x
# 
#   res <- map(objs, ~ {
#     here(params$rds_dir, .x) %>%
#       read_rds() %>%
#       subset(subset = cell_type1 == so_type)
#   })
#   
#   res <- merge(
#     x = res[[1]], 
...
sobjs <- sobjs %>%
  map(~ {
    .x %>%
      norm_sobj(regress_vars = c("S.Score", "G2M.Score")) %>%
      cluster_RNA(
        assay      = "RNA",
        resolution = 1,
        pca_meta   = F,
...
x <- sobjs %>%
  map(~ {
    ...
    #   rownames_to_column("cell_id") %>%
    #   mutate(
    #     t1_type = str_remove(t1_type, " \\(.+\\)$"),
    #     cell_type = if_else(t1_type == "unassigned", t1_type, immgen_types[t1_type])
    #   ) %>%
...
sobjs <- sobjs[match(mat_paths, names(sobjs))]
...
x <- sobjs_type[grepl("DC$|FRC$", names(sobjs_type))]

# Run clustifyr
sobjs_subtype <- imap(x, ~ {
  so_type <- so_types[[.y]]
  ...
  run_clustifyr(
    ...
    type_in = so_type,
    ref     = refs[[.y]]
    # ref     = here(params$sobj_refs[so_type])
  )
})
...
# type_ref <- d14_res1_type %>%
#   arrange(as.numeric(seurat_clusters)) %>%
#   mutate(
#     cell_type = d14.new.cluster.ids,
#     type = str_remove(type, " \\(.+\\)$")
#   ) %>%
#   select(type, cell_type) %>%
#   unique()
# 
# type_ref <- set_names(
#   x  = type_ref$cell_type,
#   nm = type_ref$type
# )
# 
# type_ref["Neutrophils"] <- "DC"
# d14_res1 <- clustify(
#   input       = d14_so,
#   cluster_col = "seurat_clusters",
#   ref_mat     = clustifyrdata::immgen_ref,
