...
run_clustifyr <- function(sobj_in, type_in = NULL, ref, resolution = 1.8, type_column = "cell_type",
  ...
  if (!is.null(type_in)) {
    ...
    res@meta.data <- res@meta.data %>%
      ...
      mutate(
        ...
        !!clust_col := if_else(!!type_col == type_in, str_c(seurat_clusters, "-", !!sub_col), !!sub_col)
      ) %>%
      column_to_rownames("cell_id")
  }
...
con_types <- c("T cell", "B cell", "epithelial")
...
subtype_refs <- cell_types %>%
  ...
  map(~ {
    ref_sobj %>%
      subset(subset = !!sym(type_column) %in% c(con_types, .x)) %>%
      seurat_ref(cluster_col = subtype_column)
  })

con_ref <- ref_sobj %>%
  subset(subset = !!sym(type_column) %in% con_types) %>%
  seurat_ref(cluster_col = type_column)
...
# Merge control ref with Xiang LEC ref
LEC_ref <- LEC_ref[rownames(LEC_ref) %in% rownames(con_ref), ]
con_ref <- con_ref[rownames(LEC_ref), ]

if (!identical(rownames(con_ref), rownames(LEC_ref))) {
  stop("Rownames do not match for control and LEC refs.")
}

LEC_ref <- cbind(con_ref, LEC_ref)

# Add new LEC ref to list
subtype_refs$LEC <- LEC_ref

# Save subtype ref matrices
params$subtype_refs %>%
  iwalk(~ {
    ref <- subtype_refs[[.y]]
