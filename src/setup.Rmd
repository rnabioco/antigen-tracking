...
calc_feat_fc <- function(sobj_in, feat = "adt_ovalbumin", data_slot = "counts", add_pseudo = F,
  ...
  res@meta.data <- res@meta.data %>%
    rownames_to_column("cell_id") %>%
    mutate(con_grp = if_else(!!sym(grp_column) %in% con_types, T, F)) %>%
    group_by(con_grp) %>%
...
run_clustifyr <- function(sobj_in, type_in, ref, resolution = 1.8, type_column = "cell_type",
  ...
  res@meta.data <- res@meta.data %>%
    rownames_to_column("cell_id") %>%
    mutate(
      !!subtype_column := if_else(!!type_column == type_in, t2_type, !!type_column),
...
LEC_ref <- str_remove(params$subtype_refs$LEC, "\\.rda$")

load(here(params$ref_dir, params$subtype_refs$LEC))

subtype_refs$LEC <- eval(parse(text = LEC_ref))

# Save DC and FRC ref matrices
params$subtype_refs[c("DC", "fibroblast")] %>%
  iwalk(~ {
    ref <- subtype_refs[[.y]]
