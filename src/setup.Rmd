...
clustify_cell_types_02 <- function(sobj_in, ref_mat, threshold) {
  ...
  if (nrow(input_markers) > 0) {
    ...
    if (nrow(input_GO) > 0) {
      ...
      if (!is.null(xlsx_name)) {
        ex_out <- str_c(xlsx_name, "_GO.xlsx")
    ...
    if (!is.null(xlsx_name)) {
      ex_out <- str_c(xlsx_name, "_markers.xlsx")
      ...
      # x <- eval(parse(text = my_file))
      # FILES_OUT <<- c(x, FILES_OUT)
      # assign()
    }
  }
...
FILES_IN <<- md5sum(here(params$ref_dir, params$type_ref)) %>%
  c(FILES_IN)
...
subtype_refs <- params$subtype_refs %>%
  map(~ {
    ...
    FILES_IN <<- md5sum(here(params$ref_dir, .x)) %>%
      c(FILES_IN)
...
# TO TEST FOR DIFFERNCES IN METADATA
walk2(sobjs, old_sobjs, ~ {

  so1 <- .x
  so2 <- .y

  ident_columns <- colnames(so1@meta.data)

  walk(ident_columns, ~ {

    .x_2 <- ifelse(.x == "subtype_clusters", "subtype_cluster", .x)

    iden <- identical(so1@meta.data[, .x], so2@meta.data[, .x_2])

    msg <- if_else(iden, str_c("IDENTICAL ", .x), str_c("NOT IDENTICAL NOT IDENTICAL NOT IDENTICAL", .x))
...
```{r "Immgen refs Shannon", eval = F, echo = F}
...
```{r "DC refs Shannon", eval = F, echo = F}
...
```{r "Shannon analysis", eval = F, echo = F}
