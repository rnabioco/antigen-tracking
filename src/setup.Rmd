...
# Compare new and old objects
# x <- names(new_sobjs) %>%
#   map_dfr( ~{
#     x     <- old_sobjs[[.x]]@meta.data
#     y     <- new_sobjs[[.x]]@meta.data
#     clmns <- colnames(x)
#     
#     clmns %>%
#       map_dfr(~ {
#         res <- identical(x[.x], y[.x])
#         
#         tibble(clmn = .x, idnt = res)
#       }) %>%
#       mutate(obj = .x)
#   })
...
old_sobjs <- so_out %>%
  map(~ {
    ...
    file.path(p, "old_objects/2020-08-17", f) %>%
      read_rds()
  })
