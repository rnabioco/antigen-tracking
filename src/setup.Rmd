...
# Add Xiang et al. LEC ref
LEC_path <- params$sobj_refs$LEC
LEC_ref <- basename(LEC_path) %>%
  str_remove(".rda$")

load(here(LEC_path))

subtype_refs$LEC <- eval(parse(text = LEC_ref))

# Save ref matrices
params$sobj_refs[c("DC", "fibroblast")] %>%
  iwalk(~ {
    ref <- subtype_refs[[.y]]
    
    save(ref, file = here(.x))
  })

save(type_ref, file = here("ref/ref_celltype_walsh.rda"))
