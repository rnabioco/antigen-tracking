...
fit_GMM <- function(sobj_in, data_column = "adt_ovalbumin") {
  ...
  res <- data.frame(
    ...
    mutate(GMM_grp = if_else(comp.1 > 0.5, "low", "high")) %>%
...
theme_cols <- c(
  ...
  "#6a51a3",  # purple
  "#c51b7d",  # pink
  palette_OkabeIto[c(1, 2, 4)]
  # "#41b6c4",  # aqua
  # "#01665e",  # teal
  # "#8c510a",  # brown
)

dark_cols <- theme_cols %>%
  darken(0.3)

light_cols <- theme_cols %>%
  lighten(0.3) %>%
  desaturate(0.2)

theme_cols <- c(theme_cols, "#000000", "#737373", dark_cols, light_cols)

# Okabe Ito color palette
# ito_cols <- c(palette_OkabeIto, "#fc4e2a")
# ito_cols <- c(palette_OkabeIto, "#ef3b2c")
# ito_cols <- c(palette_OkabeIto, "#fb6a4a")
ito_cols <- c(palette_OkabeIto[1:4], "#d7301f", palette_OkabeIto[5:6], "#6a51a3", palette_OkabeIto[7:8])
