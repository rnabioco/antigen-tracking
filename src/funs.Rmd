...
plot_features <- function(sobj_in, x = "UMAP_1", y = "UMAP_2", feature, pt_size = 0.25,
                          ...
                          max_pct = NULL, calc_cor = F, lab_size = 3.7, trim_feat = NULL,
  ...
  if (!is.null(trim_feat)) {
    short_feat <- feature %>%
      str_remove(trim_feat)
  }
  ...
  if (!is.null(min_pct) || !is.null(max_pct)) {
    counts <- counts %>%
      mutate(
        ...
        !!sym(short_feat) := if_else(!!sym(short_feat) > max_val, max_val, !!sym(short_feat)),
        !!sym(short_feat) := if_else(!!sym(short_feat) < min_val, min_val, !!sym(short_feat))
      )
  }
