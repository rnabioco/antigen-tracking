...
create_sobj <- function(matrix_dir, proj_name, hash_ids = NULL,
...
norm_sobj <- function(sobj_in, sample_order = NULL) {
  ...
  res@meta.data <- res@meta.data %>%
    rownames_to_column("cell_ids") %>%
    mutate(
      orig.ident = fct_relevel(orig.ident, sample_order)
    ) %>%
    column_to_rownames("cell_ids")
...
# Create shared nearest neighbors graph, cluster, and run UMAP using CITE-seq
# data
...
create_bubbles <- function(GO_df, plot_colors = theme_cols[c(1:2, 4, 9)],
  ...
  GO_data <- GO_df %>%
    mutate(
      ...
      term_id = str_trunc(term_id, 29, "right"),
  ...
  # Extract top 8 terms for each database
  top_GO <- GO_data %>%
    group_by(source) %>%
    arrange(p_value) %>%
  ...
  res <- GO_data %>%
    ...
    geom_point(color = plot_colors, alpha = 0.5, show.legend = F) +
    geom_text_repel(
...
find_group_markers <- function(input_grp, input_sobj, grp_col, clust_col) {
  
  res <- input_sobj %>%
    subset(!!sym(grp_col) == input_grp)
...
create_marker_umaps <- function(input_sobj, input_markers, umap_cols, add_outline = NULL) {
...
create_marker_boxes <- function(input_sobj, input_markers, type_id, box_cols,
                                cell_type = NULL, include_legend = F, all_boxes = F,
                                order_boxes = T, n_boxes = 10) {
  
  # Retrieve and format data for boxplots
  box_data <- input_sobj %>%
    FetchData(c(type_id, input_markers)) %>%
    as_tibble(rownames = "cell_id") %>%
    mutate(type = str_remove(!!sym(type_id), "^[a-zA-Z0-9_]+-"))
  
  # Filter based on input cell_type
  ...
  if (order_boxes) {
    box_data <- box_data %>%
      mutate(!!sym(type_id) := fct_reorder(!!sym(type_id), Counts, mean, .desc = T))
  }
  ...
  res <- box_data %>%
    ...
    theme_info +
    theme(
      plot.margin      = unit(c(0.9, 0.2, 0.2, 0.2), "cm"),
  ...
  if (n_type < 9 && !all_boxes) {
    res <- res +
      geom_quasirandom(size = 0.3)
    
  } else if (all_boxes) {
    res <- res +
      geom_violin(aes(fill = !!sym(type_id)), color = "black", size = 0.2) +
    ...
    res <- res +
      geom_boxplot(aes(fill = !!sym(type_id), color = !!sym(type_id)), size = 0, outlier.color = "#f0f0f0", outlier.size = 0.25) +
...
create_marker_fig <- function(input_sobj, input_markers, input_GO = NULL, type_id, 
                              input_umap, umap_color, fig_heights = c(0.46, 0.3, 0.25), 
...
set_cluster_order <- function(input_cols, input_marks) {
...
create_marker_panel_v1 <- function(input_sobj, input_cols, input_umap = NULL, clust_column, 
...
create_marker_panel_v2 <- function(input_sobj, input_markers, input_cols, grp_column, clust_column, 
  ...
  for (i in seq_along(fig_clusters)) {
    ...
    fig_cols <- input_cols[grepl(regex_type, names(input_cols))]
    ...
    ref_umap <- input_sobj %>%
      ...
      as_tibble(rownames = "cell_id") %>%
      mutate(!!sym(clust_column) := if_else(!!sym(grp_column) != type, "Other", !!sym(clust_column))) %>%
      plot_features(
        feature    = clust_column,
        plot_cols  = fig_cols,
        pt_size    = 0.1,
