...
plot_cell_count <- function(sobj_in, group_id = "type", split_id = "orig.ident",
  ...
  res <- res %>%
    ...
    geom_bar(position = bar_pos, size = 1.5) +
...
create_marker_boxes <- function(input_sobj, input_markers, type_id, box_cols, 
  ...
  if (n_type < 9) {
    ...
    res <- res +
      ...
      # geom_boxplot(aes(fill = !!sym(type_id)), size = 0.3, color = "white", outlier.color = "#f0f0f0", outlier.size = 0.25) +
      # stat_summary(fun = "median", geom = "point", shape = 21, size = 1, color = "white", fill = "white")
      # geom_boxplot(aes(fill = !!sym(type_id)), size = 0.2, color = "white", outlier.color = "#f0f0f0", outlier.size = 0.25) +
  }
