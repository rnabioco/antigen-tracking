...
CI_df <- data.frame(type = c(0.8, 0.95, 0.99)) %>%
  ...
  unnest() %>%
  dplyr::select(type, estimate, std.error, conf.low, conf.high) %>%
...
#
cacao_small <- cacao %>% filter(location %in% c("Switzerland", "Canada", "U.S.A.", "Belgium"))
...
conf_df <- data.frame(level = c(0.99, 0.95, 0.8)) %>%
  ...
  unnest() %>%
  dplyr::select(level, location, estimate, std.error, conf.low, conf.high) %>%
