...
create_fig <- function(sobj_in, cols_in, subtype_column = "cell_type2", pseudo = 0.03,
  ...
  # Set cell type order
  data_df <- sobj_in %>%
    FetchData(c(subtype_column, data_column, "UMAP_1", "UMAP_2")) %>%
  ...
  boxes <- data_df %>%
    ggplot(aes(!!sym(data_column), !!sym(box_column), fill = !!sym(box_column))) +
