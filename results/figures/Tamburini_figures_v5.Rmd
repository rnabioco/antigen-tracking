...
feats <- list(
  c("Ccl2",   "Cxcl12",   "Msr1",    "Pkm",    "Lgals3",   "Aif1"),  # Beth's list, only genes diff expressed are Pkm
  c("Mif",    "Cdc42ep3", "Malt1",   "Jak2",   "Skil",     "Nrp2"),
  c("Sema7a", "Phip",     "Cd53",    "Cd81",   "Cd9",      "Ddx6"),
  c("Il1rn",  "Zeb2",     "Ptgs2",   "Cebpb",  "Il1b",     "Fpr2"),  # These other genes are top ova high markers
  c("Ctsb",   "Clec4a1",  "Ms4a6d",  "Sod2",   "Tgfbi",    "Grn"),
  c("Nampt",  "Ly6i",     "Clec4a3", "Ms4a6c", "Ccl4",     "Lgmn"),
  c("Cybb",   "Gbp2",     "Sirpa",   "Lst1",   "BC005537", "Grina")
)
...
feats <- list(
  c("Cd53", "Ywhab", "Siva1", "Rock2", "Insm1", "Cd200"),  # Beth's list, none are diff expressed 
  c("Cd53", "Ywhab", "Siva1", "Ccl22", "Eps15", "Il21r"),
  c("Fcgr4", "Cd72", "Wfdc17", "Pou2f2", "Rgs1", "Acp5"),  # These other genes are top ova high markers
  c("C1qb", "Clec4a1", "Tmem176a", "Clec4a3", "Pltp", "Prkcb"), 
  c("Gngt2", "Basp1", "Tmem123", "Sirpa", "Lst1", "Cd300e"), 
  c("Nfkbiz", "Tcf7l2", "Lnpep", "Itgam", "Ccnd2", "Clec4b1")
)
...
## Fig 4 - day 14 FRCs {.tabset .tabset-pills}
...
gene_list <- VariableFeatures(so)

x <- map(gene_list, ~ {
  
  res <- cor(so@assays$RNA@data["Cavin1", ], so@assays$RNA@data[.x, ])
  
  res <- tibble(gene = .x, corr = res)
...
keep_cells <- so@meta.data %>%
  # filter(GMM_grp != "ova low" | subtype != "Collecting") %>%
  filter(subtype != "Collecting") %>%
  rownames()

x <- so %>%
  subset(cells = keep_cells) %>%
  FindConservedMarkers("ova high", grouping.var = "subtype")
