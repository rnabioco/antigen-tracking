...
create_fig4 <- function(sobj_in, ova_cols, feats, feat_cols, ref_cols, pt_size = 0.00001, pt_outline = 0.4, 
  ...
  ova_hist <- sobj_in@meta.data %>%
    ...
    geom_density(fill = "white", color = "white", size = 0.3, alpha = 0.8) +
...
gg_list <- fig3_params %>%
  pmap(
    ...
    data_slot   = "counts",
    ova_cols    = c("white", ova_cols_1["ova high"])
  ) %>%
  flatten()
...
plot_args <- list(
  str_c("v", 1:3),                    # Tab title
  feats,                              # Features to plot
  rep(T, 3),                          # Plot boxes
  append(cols2, rep(list(cols1), 2))  # UMAP color scale
)
