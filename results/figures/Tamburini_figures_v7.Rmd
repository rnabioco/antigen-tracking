...
# Create GMM panels
create_gmm_panels <- function(sobj_in, gmm_column = "GMM_grp", ova_cols, bar_cols, pt_size = 0.00001, pt_outline = 0.4, show_bars = T, 
                              plot_labs = c(letters[1:3], ""), legd_pos = "top", umap_margin = unit(c(0.2, 1, 0.2, 1.5), "cm"), ...) {
...
create_feat_boxes <- function(sobj_in, feats, grp_column = "subtype", box_cols, n_col = 1, median_pt = 1, ...) {
...
create_figS10 <- function(sobj_in, feat_cols, ref_cols, ova_cols, pt_size = 0.00001, median_pt = 1, pt_outline = 0.4, n_col = 4,
                          low_col = c("white", "white"), high_col_mtplyr = 1, on_top = NULL,  gmm_column = "GMM_grp",
                          gmm_grps = c("ova low", "ova high"), rel_h = c(0.5, 1, 0.4), plot_labs, box_widths = c(1, 0.2), ...) {
  ...
  res <- plot_grid(
    ...
    rel_heights    = rel_h,
    ncol           = 1,
    labels         = c("", plot_labs[3:length(plot_labs)]),
    label_fontface = "plain",
    label_size     = 18
  )
...
```{r "Fig S10", fig.width = 10, fig.height = 14}
...
genes <- list(
  ...
  map(~ {
    set_names(
      x  = get_cols()[c(1:3, 5:22)][1:length(.x)],
      nm = .x
    )
  })
...
plot_args <- list(
  ...
  plot_labs       = list(letters[1:4], letters[5:7]),
  low_col         = list(c("white", "white"), "white"),
  high_col_mtplyr = list(mtplyr_1, 1),
  rel_h           = list(c(0.5, 1, 0.4), c(0.5, 0.5, 0.4)),
  box_widths      = list(c(1, 0.2), c(1, 0.6))
)
