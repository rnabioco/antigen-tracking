...
# Create GMM panels
create_gmm_panels <- function(sobj_in, gmm_column = "GMM_grp", ova_cols, bar_cols, pt_size = 0.00001, pt_outline = 0.4,
                              show_bars = T, plot_labs = c(letters[1:3], ""), legd_pos = "top", legd_height = 0.4, legd_shape_size = 3.5,
                              umap_margin = unit(c(0.2, 1, 0.2, 1.5), "cm"), ...) {
  
  # Theme elements
  legd_guide <- guide_legend(override.aes = list(
    size    = legd_shape_size,
    shape   = 21,
    color   = "black",
    stroke  = 0.25,
    reverse = T
  ...
  ova_grp_umap <- data_df %>%
    ...
    theme(
      ...
      legend.key.height = unit(legd_height, "cm"),
  ...
  ova_hist <- sobj_in@meta.data %>%
    ...
    stat_density(geom = "point", position = "identity", size = 0, color = "#ffffff") +  # Need this for square legend symbols
  ...
  type_bar <- sobj_in@meta.data %>%
    ...
    stat_count(position = "fill", geom = "point", size = 0, color = "#ffffff") +    # Need this for square legend symbols
...
create_feat_umap_panels <- function(sobj_in, feats, ref_cols, pt_size = 0.00001, pt_outline = 0.4, panels_n_row = 2,
                                    low_col = "#ffffff", low_col_mtplyr = 2, high_col_mtplyr = 1, on_top = NULL,
                                    return_list = F, ...) {
...
create_fig5 <- function(sobj_in, feat_cols, ref_cols, ova_cols, pt_size = 0.00001, pt_outline = 0.4, median_pt = 1,
  ...
  feat_umaps <- create_feat_umap_panels(
    ...
    on_top            = on_top,
    return_list       = T,
    legend.margin     = margin(0.1, 0, 0.1, 1, "cm")
  )
...
```{r "Export meta.data", eval = T}

# Export meta.data for each Seurat object
export_meta <- function(sobj_in, file_out) {
  
  # meta.data columns to include  
  meta_cols <- c(
    "cell_id",      "orig.ident",
    "nCount_RNA",   "nFeature_RNA",
    "Percent_mito", "S.Score",
    "G2M.Score",    "Phase",
    "subtype",      "UMAP_1",
    "UMAP_2"
  )
...
sobjs %>%
  iwalk(~ {
    file_out <- here(params$rds_dir, str_c("walsh_", .y, ".tsv"))
    
    export_meta(.x, file_out)
  })
