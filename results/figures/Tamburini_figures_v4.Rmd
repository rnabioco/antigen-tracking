...
box_data <- sobj %>%
  ...
  mutate(
    ...
    GMM_grp = fct_relevel(GMM_grp, c("Low", "High"))
  )
...
# p-values
pval_df <- box_data %>%
  pivot_wider(names_from = GMM_grp, values_from = value) %>%
  group_by(name) %>%
  summarize(p_val = tidy(wilcox.test(Low, High))$p.value)

```

```{r "V1", fig.width = 10, fig.height = 12}
